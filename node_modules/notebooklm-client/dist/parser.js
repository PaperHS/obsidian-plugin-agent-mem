...
function extractAllInner(raw) {
    return parseEnvelopes(raw);
}
// ── Notebook CRUD Parsers ──
export function parseCreateNotebook(raw) {
...
export function parseListNotebooks(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return [];
    const entries = Array.isArray(inner[0]) ? inner[0] : inner;
    const notebooks = [];
...
export function parseNotebookDetail(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return { title: '', sources: [] };
    const entry = Array.isArray(inner[0]) ? inner[0] : inner;
    const title = typeof entry[0] === 'string' ? entry[0] : '';
    const sources = [];
    const sourcesArr = Array.isArray(entry[1]) ? entry[1] : [];
    for (const srcEntry of sourcesArr) {
        ...
        if (id) {
            const meta = Array.isArray(srcEntry[2]) ? srcEntry[2] : [];
...
// ── Source Parsers ──
export function parseAddSource(raw) {
    const inner = extractInner(raw);
...
export function parseListSourceThreads(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return [];
    const threads = [];
...
export function parseSourceSummary(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
...
// ── Artifact Parsers ──
export function parseGenerateArtifact(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
...
export function parseArtifacts(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return [];
    let entries = inner;
    ...
    for (const rawEntry of entries) {
        if (!Array.isArray(rawEntry))
            continue;
        const entry = (rawEntry.length === 1 && Array.isArray(rawEntry[0]))
            ? rawEntry[0]
            : rawEntry;
        ...
        if (mediaUrls.download)
            artifact.downloadUrl = mediaUrls.download;
        if (mediaUrls.stream)
            artifact.streamUrl = mediaUrls.stream;
        if (mediaUrls.hls)
            artifact.hlsUrl = mediaUrls.hls;
        if (mediaUrls.dash)
            artifact.dashUrl = mediaUrls.dash;
        if (mediaUrls.durationSeconds !== undefined)
            artifact.durationSeconds = mediaUrls.durationSeconds;
        if (mediaUrls.durationNanos !== undefined)
            artifact.durationNanos = mediaUrls.durationNanos;
        artifacts.push(artifact);
    }
    return artifacts;
}
function findMediaUrls(data, depth = 0) {
        ...
        if (data.length >= 2 && Array.isArray(data[0])) {
            ...
            if (typeof first[0] === 'string' && first[0].includes('googleusercontent.com/notebooklm/')) {
                ...
                for (const variant of data) {
                    if (!Array.isArray(variant) || typeof variant[0] !== 'string')
                        continue;
                    const url = variant[0];
                    const typeCode = variant[1];
                    if (url.includes('=m140-dv') || typeCode === 4)
                        result.download = url;
                    else if (url.includes('=m140') || typeCode === 1)
                        result.stream = url;
                    else if (url.includes('=mm,hls') || typeCode === 2)
                        result.hls = url;
                    else if (url.includes('=mm,dash') || typeCode === 3)
                        result.dash = url;
                }
                return result;
            }
...
export function findArtifactDownloadUrl(raw, artifactId) {
...
export function parseChatStream(raw) {
    ...
    for (const inner of inners) {
        if (!Array.isArray(inner))
            continue;
        const payload = Array.isArray(inner[0]) ? inner[0] : inner;
        ...
        if (meta) {
            if (typeof meta[0] === 'string' && meta[0])
                threadId = meta[0];
            if (typeof meta[1] === 'string' && meta[1])
                responseId = meta[1];
        }
    }
...
export function parseStudioConfig(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return { audioTypes: [], explainerTypes: [], slideTypes: [], docTypes: [] };
    const sections = Array.isArray(inner[0]) ? inner[0] : inner;
    function parseTypedSection(section) {
        if (!Array.isArray(section) || !Array.isArray(section[0]))
            return [];
        const items = section[0];
        return items.filter(Array.isArray).map((item) => {
            const arr = item;
            return {
...
export function parseQuota(raw) {
    const inner = extractInner(raw);
    if (!Array.isArray(inner))
        return { audioRemaining: 0, audioLimit: 0, notebookLimit: 0, sourceWordLimit: 0 };
    // Structure: [null, [?, audioLimit, notebookLimit?, sourceWordLimit], ...]
    const limits = Array.isArray(inner[1]) ? inner[1] : [];
    return {
