...
const CURL_BINARIES = ['curl-impersonate', 'curl_chrome131', 'curl_chrome116'];
...
export class CurlTransport {
    ...
    async init() {
        if (!this.binaryPath) {
            ...
            if (!found)
                throw new Error('curl-impersonate binary not found');
    ...
    static async isAvailable(binaryPath) {
        if (binaryPath) {
    ...
    static async findBinary() {
        ...
        for (const name of CURL_BINARIES) {
            ...
            if (await CurlTransport.testBinary(vendorPath))
                return vendorPath;
        }
        // Fallback: check PATH
        for (const name of CURL_BINARIES) {
            if (await CurlTransport.testBinary(name))
                return name;
        }
        return null;
    }
    static async testBinary(path) {
        try {
...
//# sourceMappingURL=transport-curl.js.map