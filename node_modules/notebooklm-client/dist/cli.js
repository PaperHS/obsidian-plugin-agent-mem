...
function buildSource(opts) {
    ...
    if (opts.topic)
        return { type: 'research', topic: opts.topic, researchMode: opts.researchMode ?? 'fast' };
...
const importSessionCmd = new Command('import-session')
    ...
    const dest = await saveSession(session, program.opts().home ? undefined : undefined);
    console.log(`Session imported to ${dest}`);
    // Verify
    try {
        const client = new NotebookClient();
        await client.connect({ transport: 'auto', session: session });
...
addBrowserOptions(addSourceOptions(analyzeCmd))
    ...
    const source = buildSource(opts);
    await withClient(opts, async (client) => {
        const result = await client.runAnalyze({ source, question: opts.question }, progressLogger);
...
const diagnoseCmd = new Command('diagnose')
    ...
    if (hasRpcOverrides) {
        try {
            const overrides = JSON.parse(readFileSync(rpcIdsPath, 'utf-8'));
...
async function resolveSkillPath(target, scope) {
...
skillCmd.addCommand(new Command('install')
...
skillCmd.addCommand(new Command('uninstall')
...
skillCmd.addCommand(new Command('status')
