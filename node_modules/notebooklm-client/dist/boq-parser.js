 ...
 */
export function stripSafetyPrefix(raw) {
 ...
 */
export function extractJsonChunks(body) {
    const chunks = [];
    ...
    while (i < lines.length) {
        ...
        if (/^\d+$/.test(line)) {
            ...
            if (nextLine?.trim()) {
                try {
                    const parsed = JSON.parse(nextLine.trim());
                    chunks.push(parsed);
                    i += 2;
                    continue;
                }
 ...
 */
export function parseEnvelopes(raw) {
    ...
    for (const chunk of chunks) {
        ...
        for (const env of chunk) {
            if (!Array.isArray(env) || env[0] !== 'wrb.fr')
                continue;
            if (typeof env[2] === 'string') {
                try {
                    const parsed = JSON.parse(env[2]);
...
//# sourceMappingURL=boq-parser.js.map